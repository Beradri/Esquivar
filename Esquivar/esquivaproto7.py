...
# Inicializamos la posición, velocidad y tiempo de caída de los objetos
objetos = [
...
while True:
    ...
    # Lógica del juego
    # Mostrar "Has perdido" si vidas es 0
    if vidas == 0:
        pantalla.blit(coras0, (0, 0))
        perdido_texto = fuente.render("Has perdido", True, (255, 255, 255))
        pantalla.blit(perdido_texto, (ancho_pantalla // 2 - 80, alto_pantalla // 2 - 20))
    ...
    for objeto in objetos:
        ...
        if objeto["rect"].colliderect(pygame.Rect(pos_persona, persona.get_size())):
            # Reposicionar objeto arriba de la pantalla
            objeto["rect"].topleft = (random.randint(0, ancho_pantalla - objeto["imagen"].get_width()), 0)
            vidas -= 1
            if vidas == 0:
                # Esto ya está manejado arriba para mostrar "Has perdido"
                pass
    ...
    # Aumentar velocidad de caída cada 5 segundos
    if tiempo_partida // 5000 > (tiempo_partida - reloj.tick(30)) // 5000:
        for objeto in objetos:
            objeto["velocidad"] += 4  # Aumento más significativo de velocidad
    ...
    # Dibujar tiempo de partida en negro
